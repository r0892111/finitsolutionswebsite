...
import { getDropboxRedirectUri } from '@/lib/oauth';
...
function DropboxCallbackContent() {
    ...
    const handleCallback = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          // Check if error is about code being expired/already used
          // This can happen if React Strict Mode causes duplicate calls
          // If the code was already used successfully, check if integration exists
          if (errorMessage.includes("code doesn't exist or has expired")) {
            try {
              ...
              if (integrationType) {
                ...
                if (existingIntegration && existingIntegration.status === 'connected') {
                  ...
                  toast({
                    title: 'Success',
                    description: 'Dropbox account connected successfully',
                  });
                  router.replace('/portal/integrations/dropbox/callback?success=true');
        ...
        setStatus('error');
        let message = 'Failed to connect Dropbox account';
        ...
        if (message.includes("code doesn't exist or has expired")) {
          setErrorMessage('The authorization code has expired or was already used. Please try connecting again.');
        }
        
        toast({
          title: 'Error',
          description: message.includes("code doesn't exist or has expired") 
  ...
  const handleRetry = async () => {
    try {
      ...
      // Get Dropbox integration type ID
      ...
      if (typeError || !integrationType) {
        throw new Error('Dropbox integration type not found');
      }
      ...
      <div className="text-center">
        {status === 'loading' && (
          <>
            ...
            <h2 className="finit-h2 text-primary mb-2">Connecting Dropbox...</h2>
