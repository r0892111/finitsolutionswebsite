...
export function LightQuizSummary({ data, onRestart }: LightQuizSummaryProps) {
  ...
  return (
    // ✅ New wrapper to offset a fixed navbar. Uses CSS var --nav-h with 72px fallback.
    <section
      id="scan-result"
      ...
      <div className="space-y-6">
        <Card className="border-2 border-blue-200">
          <CardHeader className="text-center">
            <CardTitle id="scan-title" className="text-3xl mb-2">
          ...
          <CardContent className="space-y-8">
            ...
            {data.ai_readiness && (
              <div className="text-center py-6">
                ...
                <div
                  className={`inline-block px-6 py-3 rounded-full text-2xl font-bold ${getReadinessColor(
                    data.ai_readiness
                  )}`}
                >
                  {data.ai_readiness}
                ...
                <div className="space-y-3 text-gray-700">
                  ...
                  {data.summary.current_state && (
                    <p className="leading-relaxed">
                      <strong>Huidige staat:</strong> {data.summary.current_state}
                    </p>
                  )}
                  {data.summary.ai_potential && (
                    <p className="leading-relaxed">
                      <strong>AI potentieel:</strong> {data.summary.ai_potential}
                    </p>
                  )}
            ...
            {data.recommendation_tagline && (
              <div className="bg-gradient-to-r from-green-50 to-blue-50 p-5 rounded-xl border-l-4 border-green-500">
                <p className="text-gray-800 font-medium text-lg">💡 {data.recommendation_tagline}</p>
              </div>
            )}
