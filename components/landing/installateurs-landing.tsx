...
const painPoints: PainPoint[] = [
  { icon: ClipboardList, text: "Projectplanning die je elke dag bijstelt in Excel" },
  { icon: FileText, text: "Materiaallijsten die je handmatig samenstelt per project" },
  { icon: Clock, text: "Opvolging van klanten en onderaannemers kost je uren per week" },
  { icon: CalendarCheck, text: "Afspraken inplannen met klanten is een eindeloos heen-en-weer" },
  { icon: Users, text: "Groeien lukt niet zonder nog iemand op kantoor" },
  { icon: TrendingDown, text: "Projecten lopen uit omdat je overzicht mist" },
];
...
const solutions: Solution[] = [
  {
    icon: FileText,
    title: "Offertes en materiaallijsten maken zichzelf",
    description: "Klant vraagt een offerte? Het systeem stelt hem samen op basis van je standaardprijzen en materialen. Verstuurd binnen minuten.",
  },
  {
    icon: CalendarCheck,
    title: "Je projectplanning werkt zichzelf bij",
    description: "Afspraken, deadlines en opvolging worden automatisch ingepland. Je team en je klanten weten altijd wat de status is.",
  },
  {
    icon: RefreshCw,
    title: "Opvolging die nooit stopt",
    description: "Na elke fase: automatische statusupdate naar de klant, herinnering voor de volgende stap, factuur op tijd. Projecten die vanzelf lopen.",
  },
];
...
const storyLines: StoryLine[] = [
  { icon: Clock, text: "Je bent op een werf bezig met een installatie." },
...
const HandDrawnCheck = ({ className = "" }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M3.5 13.5C5 15 8.5 18.5 9.5 19.5C12 15 16 9 21 4.5" stroke="#1A2D63" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);
...
const LogoCarousel = ({ carouselRef, className, logoSize = 74, svgTopPercent = 50, spacingMultiplier = 1, pathD, durationSeconds }: LogoCarouselProps) => {
  ...
  useEffect(() => {
    ...
    const handleResize = () => { const newWidth = window.innerWidth; if (newWidth === lastWidth) return; lastWidth = newWidth; clearTimeout(resizeTimeout); resizeTimeout = setTimeout(() => { setResizeKey((prev) => prev + 1); }, 100); };
  ...
  useEffect(() => {
    ...
    logos.forEach((logo, index) => {
      ...
      gsap.set(logo, { xPercent: -50, yPercent: -50, opacity: 1, force3D: true });
      const tween = gsap.fromTo(logo, { motionPath: { path, align: path, alignOrigin: [0.5, 0.5], autoRotate: true, start: startProgress, end: startProgress } }, { motionPath: { path, align: path, alignOrigin: [0.5, 0.5], autoRotate: true, start: startProgress, end: startProgress + 1 }, duration: animationDuration, ease: "none", repeat: -1, immediateRender: true, force3D: true });
      tweensRef.current.push(tween);
    });
    return () => { tweensRef.current.forEach((tween) => tween.kill()); tweensRef.current = []; };
  }, [isMounted, resizeKey, spacingMultiplier, durationSeconds]);

  const setRefs = useCallback((node: HTMLDivElement | null) => {
    (containerRef as React.MutableRefObject<HTMLDivElement | null>).current = node;
    if (carouselRef && "current" in carouselRef) { (carouselRef as React.MutableRefObject<HTMLDivElement | null>).current = node; }
  }, [carouselRef]);

  return (
    <div ref={setRefs} className={className ?? "logo-carousel absolute inset-0 pointer-events-none overflow-visible hidden lg:block"} style={{ zIndex: 6 }}>
      <svg ref={svgRef} className="absolute left-0" style={{ width: "100%", height: "auto", aspectRatio: `${CURVE_ASPECT_RATIO}`, top: `${svgTopPercent}%`, transform: "translateY(-50%)" }} viewBox="0 0 1600 900" preserveAspectRatio="xMidYMid meet">
        <path id="logoMotionPath" d={pathD ?? "M -100,612 C 100,650 300,720 500,770 C 700,810 850,810 950,780 C 1100,730 1250,550 1400,350 C 1500,220 1600,120 1700,80"} fill="none" stroke="transparent" />
      </svg>
      {integrationLogos.map((logo, index) => (
        <div key={`floating-${logo.name}-${index}`} className="floating-logo absolute" style={{ width: `${logoSize}px`, height: `${logoSize}px`, left: 0, top: 0, opacity: 0, willChange: "transform" }}>
...
const sectionDividerData = [
  { fill: "M0,70 C480,130 960,-10 1920,50", navy: "M0,62 C480,110 960,-18 1920,34 L1920,60 C960,4 480,146 0,78 Z", light: "M0,78 C480,146 960,4 1920,60 L1920,74 C960,14 480,164 0,88 Z" },
  { fill: "M0,30 C320,110 640,120 960,60 C1280,0 1600,-10 1920,80", navy: "M0,22 C320,92 640,100 960,50 C1280,-10 1600,-16 1920,66 L1920,90 C1600,4 1280,10 960,70 C640,134 320,126 0,38 Z", light: "M0,38 C320,126 640,134 960,70 C1280,10 1600,4 1920,90 L1920,102 C1600,18 1280,22 960,80 C640,146 320,142 0,48 Z" },
  { fill: "M0,85 C320,10 640,-5 960,55 C1280,115 1600,130 1920,35", navy: "M0,77 C320,-8 640,-25 960,45 C1280,107 1600,114 1920,21 L1920,45 C1600,144 1280,125 960,65 C640,9 320,26 0,93 Z", light: "M0,93 C320,26 640,9 960,65 C1280,125 1600,144 1920,45 L1920,57 C1600,158 1280,137 960,75 C640,21 320,40 0,103 Z" },
  { fill: "M0,55 C240,110 480,110 720,60 C960,10 1200,10 1440,60 C1680,110 1800,110 1920,55", navy: "M0,47 C240,92 480,90 720,50 C960,2 1200,4 1440,50 C1680,92 1800,90 1920,41 L1920,65 C1800,124 1680,126 1440,70 C1200,24 960,20 720,70 C480,126 240,124 0,63 Z", light: "M0,63 C240,124 480,126 720,70 C960,20 1200,24 1440,70 C1680,126 1800,124 1920,65 L1920,77 C1800,136 1680,140 1440,80 C1200,38 960,32 720,80 C480,138 240,138 0,73 Z" },
];

const SectionDivider = ({ fromColor, toColor, variant = 0 }: { fromColor: string; toColor: string; variant?: number }) => {
  const data = sectionDividerData[variant % sectionDividerData.length];
  const fillPath = `${data.fill} L1920,160 L0,160 Z`;
  return (
    <div className="relative w-full" style={{ backgroundColor: fromColor, marginTop: -1, marginBottom: -1 }} aria-hidden="true">
      <svg viewBox="0 -40 1920 200" preserveAspectRatio="none" className="w-full block h-[50px] md:h-[75px] lg:h-[100px]" xmlns="http://www.w3.org/2000/svg" style={{ overflow: "visible" }}>
        <path d={fillPath} fill={toColor} /><path d={data.navy} fill="#1A2D63" /><path d={data.light} fill="#7B8DB5" />
      </svg>
    </div>
  );
};
...
const materialItems = [
  "Leidingwerk 22mm (12m)",
  "Koppelingen T-stuk (8x)",
  "Isolatiemateriaal (15m)",
  "Bevestigingsmateriaal",
  "Afdichtingstape",
];

// ============================================
// TIMELINE DOT (fills on intersection)
// ============================================

const TimelineDot = ({ isActive }: { isActive: boolean }) => (
  <div
    className={`w-4 h-4 rounded-full border-2 border-[#1A2D63] transition-colors duration-500 ${
      isActive ? "bg-[#1A2D63]" : "bg-transparent"
    }`}
  />
);
...
export function InstallateursLanding() {
  ...
  useEffect(() => {
    const handleScroll = () => { const scrollY = window.scrollY; setNavScrollProgress(Math.min(scrollY / 100, 1)); setShowStickyMobileCTA(scrollY > window.innerHeight * 0.3); };
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-[#1A2D63] font-instrument selection:bg-[#B8C5E6] selection:text-[#1A2D63] overflow-x-hidden">
      ...
      {/* NAV */}
      <nav className="fixed top-0 left-0 right-0 z-40" style={{ background: navScrollProgress > 0 ? `rgba(253, 251, 247, ${0.82 * navScrollProgress})` : "transparent", backdropFilter: navScrollProgress > 0 ? `blur(${14 * navScrollProgress}px)` : "none", WebkitBackdropFilter: navScrollProgress > 0 ? `blur(${14 * navScrollProgress}px)` : "none", borderBottom: `1px solid rgba(26, 45, 99, ${0.1 * navScrollProgress})`, transition: "background 0.3s, backdrop-filter 0.3s, border-bottom 0.3s" }}>
        <div className="max-w-[1400px] mx-auto flex items-center justify-between px-4 sm:px-6 relative" style={{ paddingTop: `${12 + (1 - navScrollProgress) * 12}px`, paddingBottom: `${12 + (1 - navScrollProgress) * 12}px`, transition: "padding 0.3s" }}>
        ...
        <div className="hidden md:flex container mx-auto px-4 sm:px-6 md:px-10 lg:px-12 xl:px-16 items-center justify-center pt-28 lg:pt-32 pb-8 w-full relative z-10">
          <div className="relative z-10 text-center max-w-5xl">
            <motion.div className="mb-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
              <h1 className="font-newsreader text-5xl lg:text-6xl xl:text-[4.25rem] leading-[1.1] tracking-tight text-[#1A2D63]">
                ...
                <span className="relative inline-block">
                  <span className="relative z-10">vanzelf lopen.</span>
            ...
            <motion.p className="font-instrument text-lg lg:text-xl text-[#475D8F] leading-relaxed max-w-2xl mx-auto mb-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}>
              Wij automatiseren de administratie van installateurs. Projectplanning, materiaallijsten, offertes en opvolging draaien op de achtergrond. Jij focust op de installatie.
            </motion.p>
            <motion.div className="flex flex-row items-center justify-center gap-3 mb-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }}>
              <button type="button" onClick={() => { openForm(); pushEvent("cta_click", { cta_label: "hero_calendly", location: "lp_installateurs_hero" }); }} className="group bg-[#1A2D63] text-white px-7 py-3.5 rounded-full text-base font-medium flex items-center justify-center gap-2.5 hover:bg-[#2A4488] transition-colors shadow-lg shadow-[#1A2D63]/10">
                <Calendar className="w-4 h-4" /><span>Plan een kennismakingsgesprek</span><ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </button>
            </motion.div>
            ...
            <motion.div className="flex flex-col items-start gap-2 text-[#1A2D63]/70 text-lg mx-auto w-fit text-left" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.7 }}>
              <span className="flex items-center gap-2.5 whitespace-nowrap"><HandDrawnCheck className="w-6 h-6 flex-shrink-0" />We leren eerst jouw manier van werken kennen</span>
              <span className="flex items-center gap-2.5 whitespace-nowrap"><HandDrawnCheck className="w-6 h-6 flex-shrink-0" />We bepalen waar AI het meeste tijd oplevert</span>
              <span className="flex items-center gap-2.5 whitespace-nowrap"><HandDrawnCheck className="w-6 h-6 flex-shrink-0" />Fractie van de kost van een echte werknemer</span>
            </motion.div>
          </div>
        </div>
        ...
        <div className="md:hidden relative z-10 px-4 sm:px-6 pt-28 pb-6">
          <div className="text-center max-w-[22rem] sm:max-w-[28rem] mx-auto">
            <motion.h1 className="font-newsreader text-[2.5rem] sm:text-5xl leading-[1.1] tracking-tight text-[#1A2D63] mb-5" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
              Projecten die vanzelf{" "}
              <span className="relative inline-block">
                <span className="relative z-10">lopen.</span>
                <svg className="absolute -bottom-1 left-0 w-full h-[0.35em] z-0" viewBox="0 0 200 20" preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 14 Q40 4 100 12 Q160 18 197 8" stroke="#1A2D63" strokeOpacity="0.15" strokeWidth="10" strokeLinecap="round" fill="none" /></svg>
              </span>
            </motion.h1>
            <motion.p className="font-instrument text-base sm:text-[17px] text-[#475D8F] leading-relaxed max-w-lg mx-auto mb-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}>
              Wij automatiseren de administratie van installateurs. Projectplanning, materiaallijsten, offertes en opvolging draaien op de achtergrond.
            </motion.p>
            <motion.div className="flex flex-col items-center justify-center gap-3 mb-5" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }}>
              <button type="button" onClick={() => { openForm(); pushEvent("cta_click", { cta_label: "hero_calendly", location: "lp_installateurs_hero" }); }} className="group w-full bg-[#1A2D63] text-white px-6 py-3.5 rounded-full text-[15px] font-medium flex items-center justify-center gap-2.5 hover:bg-[#2A4488] transition-colors shadow-lg shadow-[#1A2D63]/10">
                <Calendar className="w-4 h-4" /><span>Plan een kennismakingsgesprek</span><ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </button>
            </motion.div>
            ...
            <motion.div className="flex flex-col items-start gap-2.5 text-[#1A2D63]/70 text-base mx-auto w-fit text-left" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.7 }}>
              <span className="flex items-center gap-2.5"><HandDrawnCheck className="w-6 h-6 flex-shrink-0" />We leren eerst jouw manier van werken kennen</span>
              <span className="flex items-center gap-2.5"><HandDrawnCheck className="w-6 h-6 flex-shrink-0" />We bepalen waar AI het meeste tijd oplevert</span>
      ...
      <section id="solutions" className="pt-8 md:pt-12 pb-8 md:pb-10 px-6 md:px-12 bg-[#FDFBF7]">
        <div className="max-w-[1100px] mx-auto">
          <motion.div className="text-center mb-8 md:mb-10" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.5 }}>
            <h2 className="font-newsreader text-3xl sm:text-4xl md:text-5xl text-[#1A2D63] leading-[1.15] mb-4">
              Wat als je projecten gewoon{" "}
              <span className="relative inline-block"><span className="relative z-10">vanzelf</span><svg className="absolute -bottom-1 left-0 w-full h-[0.35em] z-0" viewBox="0 0 200 20" preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 14 Q40 4 100 12 Q160 18 197 8" stroke="#1A2D63" strokeOpacity="0.15" strokeWidth="10" strokeLinecap="round" fill="none" /></svg></span>{" "}lopen?
            </h2>
            <p className="text-[#1A2D63]/60 text-lg md:text-xl max-w-xl mx-auto">AI neemt het repetitieve werk over. Jij focust op de installatie.</p>
          </motion.div>
          ...
          <motion.div className="flex flex-col sm:flex-row items-stretch justify-center gap-4 md:gap-6 max-w-2xl mx-auto mb-10 md:mb-12" initial={{ opacity: 0, scale: 0.96 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.5 }}>
            <div className="flex-1 bg-[#1A2D63]/[0.04] rounded-2xl p-7 md:p-8 text-center border border-[#1A2D63]/[0.08] shadow-[0_1px_0_0_rgba(26,45,99,0.1),0_4px_6px_-1px_rgba(26,45,99,0.15),0_10px_20px_-3px_rgba(26,45,99,0.2),0_20px_40px_-8px_rgba(26,45,99,0.15)]">
              ...
              <p className="text-4xl md:text-5xl font-newsreader text-[#1A2D63] font-semibold leading-none">20+</p>
            ...
            <div className="flex-1 bg-[#1A2D63] rounded-2xl p-7 md:p-8 text-center shadow-xl shadow-[#1A2D63]/15">
              ...
              <p className="text-4xl md:text-5xl font-newsreader text-white font-semibold leading-none">4</p>
          ...
          <motion.div className="mt-8 md:mt-10" initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.4, delay: 0.35 }}>
            <button type="button" onClick={() => { openForm(); pushEvent("cta_click", { cta_label: "breadth_card_calendly", location: "lp_installateurs_solutions" }); }} className="group flex items-center gap-5 md:gap-6 max-w-2xl mx-auto rounded-3xl border-2 border-dashed border-[#1A2D63]/20 hover:border-[#1A2D63]/40 px-8 py-7 md:px-10 md:py-8 transition-all hover:bg-white/60 cursor-pointer w-full text-left">
              <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-[#1A2D63]/[0.08] group-hover:bg-[#1A2D63]/[0.15] flex items-center justify-center shrink-0 transition-colors"><Plus className="w-7 h-7 md:w-8 md:h-8 text-[#1A2D63]" /></div>
              <div className="flex-1 min-w-0">
                <p className="font-newsreader text-xl md:text-2xl text-[#1A2D63] leading-snug mb-1">En alles wat repetitief is in jouw installatiebedrijf</p>
                <p className="text-[#1A2D63]/60 text-base md:text-lg">Vertel ons wat je team dagelijks doet — wij laten zien wat er kan.</p>
              </div>
              <ArrowRight className="w-5 h-5 md:w-6 md:h-6 text-[#1A2D63]/30 group-hover:text-[#1A2D63]/60 shrink-0 group-hover:translate-x-1 transition-all" />
            </button>
          </motion.div>
        </div>
      </section>
      ...
      <section id="how-it-works" className="pt-8 md:pt-12 pb-12 md:pb-16 px-6 md:px-12 bg-[#FDFBF7]">
        <div className="max-w-[1100px] mx-auto">
          <motion.div className="text-center mb-8 md:mb-10" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.5 }}>
            <h2 className="font-newsreader text-3xl sm:text-4xl md:text-5xl text-[#1A2D63] leading-[1.15] mb-4">
              AI helpt in elke{" "}
              <span className="relative inline-block"><span className="relative z-10">projectfase</span><svg className="absolute -bottom-1 left-0 w-full h-[0.35em] z-0" viewBox="0 0 200 20" preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 14 Q40 4 100 12 Q160 18 197 8" stroke="#1A2D63" strokeOpacity="0.15" strokeWidth="10" strokeLinecap="round" fill="none" /></svg></span>
            </h2>
            <p className="text-[#1A2D63]/60 text-lg md:text-xl max-w-xl mx-auto">
              Van offerte tot nazorg. Alles loopt automatisch door.
            </p>
          </motion.div>
          ...
          <motion.div
            className="mt-8 md:mt-10 max-w-lg mx-auto"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-40px" }}
            transition={{ duration: 0.4, delay: 0.3 }}
          >
            <div className="bg-white rounded-3xl p-7 md:p-8 border border-[#1A2D63]/[0.06] shadow-[0_1px_0_0_rgba(26,45,99,0.1),0_4px_6px_-1px_rgba(26,45,99,0.15),0_10px_20px_-3px_rgba(26,45,99,0.2),0_20px_40px_-8px_rgba(26,45,99,0.15)]">
              <div className="flex items-center gap-3 mb-5">
                <div className="w-10 h-10 rounded-2xl bg-[#1A2D63]/10 border border-[#1A2D63]/25 flex items-center justify-center">
                  <Package className="w-5 h-5 text-[#1A2D63]" />
                </div>
                <div>
                  <p className="font-newsreader text-xl md:text-2xl text-[#1A2D63] leading-snug">
                    Nooit meer materiaal vergeten op de werf
                  </p>
                </div>
              </div>
              <div className="space-y-2.5 pl-1">
                {materialItems.map((item, index) => (
                  ...
                  >
                    <div className="w-5 h-5 rounded border-2 border-[#1A2D63]/20 flex items-center justify-center shrink-0">
                      <Check className="w-3.5 h-3.5 text-[#1A2D63]" />
                    </div>
                    <span className="text-[#1A2D63]/70 text-base md:text-lg">{item}</span>
                  </motion.div>
                ))}
              </div>
              <p className="text-[#1A2D63]/50 text-sm mt-4">
                AI stelt de lijst samen op basis van je offerte. Automatisch.
              </p>
            </div>
          </motion.div>
        ...
        <div className="max-w-[800px] mx-auto relative z-10">
          <motion.div className="flex flex-col md:flex-row items-center gap-6 md:gap-8" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.5 }}>
            ...
            <div className="text-center md:text-left">
              <blockquote className="font-instrument text-lg md:text-xl text-white leading-relaxed mb-3">&ldquo;Snelle oplevering, sympathieke gasten. Ik wist niet dat AI vandaag al zoveel werk kon overnemen! &rdquo;</blockquote>
            ...
            <Accordion type="single" collapsible className="w-full">
              {faqItems.map((item, index) => (
                <AccordionItem key={index} value={`faq-${index}`} className="border-b border-[#1A2D63]/[0.08] last:border-b-0 md:last:border-b md:border-[#1A2D63]/10">
                  <AccordionTrigger className="py-4 sm:py-5 md:py-6 text-left text-[#1A2D63] font-instrument text-base sm:text-lg md:text-xl font-medium hover:no-underline hover:text-[#475D8F] transition-colors [&>svg]:h-4 [&>svg]:w-4 sm:[&>svg]:h-5 sm:[&>svg]:w-5 [&>svg]:text-[#475D8F] [&>svg]:shrink-0 [&>svg]:ml-3 gap-2">{item.question}</AccordionTrigger>
                  <AccordionContent className="text-[#1A2D63]/65 text-[15px] sm:text-base md:text-[17px] leading-[1.7] pb-5 sm:pb-6 md:pb-7 space-y-3.5">
                    {item.answer.map((block, i) => {
                      ...
                      if (block.type === "list") return (<ul key={i} className="space-y-2 pl-1">{block.items.map((entry, j) => (<li key={j} className="flex items-start gap-2.5"><span className="mt-[9px] h-1.5 w-1.5 rounded-full bg-[#1A2D63]/30 shrink-0" /><span>{entry}</span></li>))}</ul>);
                      return null;
                    })}
                  </AccordionContent>
                </AccordionItem>
